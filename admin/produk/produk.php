    ...
    <div class="row">
      ...
      <main class="col-md-9 ms-sm-auto col-lg-10 px-md-4">
        ...
        <?php } else if (isset($_GET['failed'])) { ?>
          <div class="alert alert-danger alert-dismissible <?= (isset($_GET['success'])) ? 'show' : '' ?>" role="alert">
            ...
            <tbody class="w-100" style="max-height: 80vh; box-sizing: border-box; overflow: auto;">
              ...
              foreach ($produk as $row) : ?>
                <tr d-flex flex-column justify-content-center>
                  ...
                  <td><img src="../../assets/produk-image/<?= $row['gambar'] ?>"></td>
                  <td><?= $row['judul'] ?></td>
                  <td><?= $row['kategori'] ?></td>
                  <td>Rp. <?= number_format($row['harga'], 0, ',', '.') ?></td>
                  <td class="action text-center">
                    <a href="delete-produk.php?id=<?= $row['id_produk'] ?>" class="btn btn-danger" onclick="return confirm('Anda ingin menghapus menu <?= $row['judul'] ?>')">
